...
import { Award, ShieldCheck, Mail, Phone, MapPin, ArrowUpRight } from 'lucide-react';

export default function Footer({ setView }) {
  return (
    <footer className="bg-[#EFECE3] border-t border-[#E2DDD1] text-[#2C2824] pt-12 pb-8">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
        {/* Brand Col */}
        <div className="space-y-4 md:col-span-1">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-[#1B3B2B] border border-[#C5A059] flex items-center justify-center text-[#C5A059]">
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-xs text-[#554F47]">
        ...
        {/* Contact Info */}
        <div className="space-y-3 text-xs text-[#554F47]">
          <h4 className="font-serif font-bold text-sm text-[#1C1917] uppercase tracking-wider">Helpdesk</h4>
          <p className="flex items-center gap-2">
            <Mail className="w-4 h-4 text-[#A37B34]" /> support@nms-platform.com
          </p>
          <p className="flex items-center gap-2">
            <Phone className="w-4 h-4 text-[#A37B34]" /> +1 (800) 555-NMS-HELP
          </p>
          <p className="flex items-center gap-2">
            <MapPin className="w-4 h-4 text-[#A37B34]" /> Financial District, Suite 400
          </p>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-6 pt-6 border-t border-[#D8CEBE] flex flex-wrap items-center justify-between text-xs text-[#736C63] gap-4">
