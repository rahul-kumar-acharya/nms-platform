  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h3 className="text-xl font-serif font-bold text-[#1C1917]">Membership Plans & Pricing</h3>
          <p className="text-xs text-[#736C63]">Configure plan prices, referral bonuses & binary pair payouts</p>
        </div>

        {activeTab === 'list' ? (
          <button onClick={openCreateForm} className="btn-gold text-xs">
      ...
      {activeTab === 'list' ? (
        loading ? (
          ...
          {plans.map(p => (
            <div key={p.id} className="glass-card p-6 border-[#C5A059] flex flex-col justify-between space-y-4">
              <div>
                ...
                <h4 className="text-2xl font-serif font-extrabold text-[#1C1917] mt-3">{p.name}</h4>
                <p className="text-3xl font-extrabold text-[#1B3B2B] mt-1">₹{parseFloat(p.price).toLocaleString()}</p>
                <p className="text-xs text-[#554F47] mt-3">{p.description || 'Standard membership tier'}</p>
              </div>

              <div className="pt-4 border-t border-[#E2DDD1] space-y-2 text-xs">
        ...
        <div className="glass-card p-8 max-w-2xl mx-auto space-y-6">
          ...
          <form onSubmit={handleSavePlan} className="space-y-4">
            <div>
              ...
              <input type="text" placeholder="e.g. Plan C (Elite)" value={name} onChange={e => setName(e.target.value)} required className="form-input" />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="form-label">Price (₹)</label>
                <input type="number" placeholder="5000" value={price} onChange={e => setPrice(e.target.value)} required className="form-input" />
              </div>
              <div>
                <label className="form-label">Referral Bonus (₹)</label>
                <input type="number" placeholder="800" value={referralBonus} onChange={e => setReferralBonus(e.target.value)} required className="form-input" />
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="form-label">Binary Pair Payout (₹)</label>
                <input type="number" placeholder="1500" value={binaryPayout} onChange={e => setBinaryPayout(e.target.value)} required className="form-input" />
              </div>
              <div>
                <label className="form-label">Daily Capping Limit (₹)</label>
                <input type="number" placeholder="15000" value={dailyCapping} onChange={e => setDailyCapping(e.target.value)} required className="form-input" />
              </div>
            </div>

            <div>
              <label className="form-label">Plan Description</label>
              <textarea placeholder="Specify plan perks and benefits..." value={description} onChange={e => setDescription(e.target.value)} className="form-input h-24" />
            </div>
